...
data(sst)
nino <- cast2mlydata(sst, year.var = 'year', month.var = 'month',
                     value.var = c('nino12', 'nino3', 'nino34','nino4'))
nino <- normalize(nino, mode = 'sd1')
nino <- expand(nino, k = 1, direction = 'both')
nino <- na.trim(nino)

ninoel <- window(nino, year = c(1983, 1998, 2010))
ninoel <- melt(ninoel)
library(lattice)
xyplot(value~month|variable, group = year, data = ninoel, type = 'l')
